...
//     }
//
// Any other auxillary keys and values as siblings to the "tokens" key
...
// seriously gone wrong, be assidious in reporting as much information
// as possible.
...
func (t *tokenDb) Poll() (err error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			// loading.  Being that, silence the errot.
			return nil
		}
	...
	if nonfatale != nil {
		...
		// for the caller, because it's likely the caloler will
		// want to do something extreme in event of Poll()
		// errors, which otherwise tend to arise from serious
		// condirions preventing data base manipulation like
		// "out of disk".
		return nil
	}
	...
	// Remove last_error and tokens.rej file as the persistance
